...
# 定义了基于 dm_env 框架的车联网环境类vehicularNetworkEnv，包含初始化、重置、步骤交互等方法，核心通过全局动作空间处理车辆与边缘节点的资源分配、任务卸载等逻辑，并实现基于全局动作的奖励计算。
# 为强化学习提供全局视角的车联网环境，支持从系统整体优化资源分配和任务卸载策略，用于实验验证全局决策的性能。
